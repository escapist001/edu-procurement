...
4. **`build.py`** — объединяет всё в `rows.json` и считает агрегаты (в том числе медиану снижения по сегментам и регионам).
5. **`prepare_map.py`** — готовит geojson субъектов РФ для полигональной карты.
...
python build.py                        # → rows.json + агрегаты
...
Дальше: досье поставщиков-победителей из реестра контрактов (кто и с каким снижением выигрывает), коды ОКПД2/КТРУ, накопление истории.
